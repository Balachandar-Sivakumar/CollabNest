<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta content="width=device-width, initial-scale=1" name="viewport" />
  <title>Welcome to CollabPlatform - Collaborative Work Platform</title>
  <script src="https://cdn.tailwindcss.com"></script>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600&display=swap" rel="stylesheet" />
  <script defer src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/js/all.min.js"></script>

  <!-- AOS animation CSS -->
  <link href="https://unpkg.com/aos@2.3.1/dist/aos.css" rel="stylesheet" />

  <style>
    body {
      font-family: 'Inter', sans-serif;
    }
  </style>
</head>
<body class="bg-white text-gray-900 flex flex-col min-h-screen">
  <!-- Navbar -->
  <nav class="flex items-center justify-between px-6 py-4 border-b border-gray-200 shadow-sm" data-aos="fade-down">
    <div class="flex items-center space-x-3">
      <div class="w-10 h-10 rounded overflow-hidden">
        <img
          src="https://sdmntprnorthcentralus.oaiusercontent.com/files/00000000-e454-622f-8e2d-e85930ca57ca/raw?se=2025-05-23T23%3A07%3A44Z&sp=r&sv=2024-08-04&sr=b&scid=0347ee2b-e592-56de-8757-bb7ae920edac&skoid=add8ee7d-5fc7-451e-b06e-a82b2276cf62&sktid=a48cca56-e6da-484e-a814-9c849652bcb3&skt=2025-05-23T13%3A08%3A22Z&ske=2025-05-24T13%3A08%3A22Z&sks=b&skv=2024-08-04&sig=ZNSHIzBDzN2Lx2KqIjOpe73Xih6u5VMqAhbofCEpL/I%3D"
          alt="CollabNest Logo"
          class="w-full h-full object-contain"
        />
      </div>
      <h1 class="text-2xl font-bold">
        <span class="text-blue-600">Collab</span><span class="text-blue-600">Nest</span>
      </h1>
    </div>
    <div class="space-x-6 text-sm font-semibold">
      <a href="{{ route('home') }}" class="hover:text-blue-600 transition cursor-pointer">Home</a>
      <a href="{{ route('how-it-works') }}" class="hover:text-blue-600 transition cursor-pointer">How it works</a>
      <a href="{{ route('explore-projects') }}" class="hover:text-blue-600 transition cursor-pointer">Explore Projects</a>
      <a href="{{ route('find-talent') }}" class="text-blue-600 transition cursor-pointer">Find Talent</a>
      <a href="{{ route('help') }}" class="hover:text-blue-600 transition cursor-pointer">Help</a>
    </div>

    <div class="space-x-4">
      <a href="/navlogin" class="text-blue-600 font-semibold hover:underline">Log In</a>
      <a href="/register" class="bg-blue-600 text-white px-4 py-2 rounded-md font-semibold hover:bg-blue-700 transition">Sign Up</a>
    </div>
  </nav>

  <!-- Find Talent Section -->
  <section class="max-w-7xl mx-auto px-6 py-16">
    <div class="text-center mb-12" data-aos="fade-up">
      <h2 class="text-4xl font-bold text-blue-600 mb-4">Find Top Talent for Your Projects</h2>
      <p class="text-gray-700 max-w-3xl mx-auto text-lg">
        Connect with skilled professionals across various domains. Whether you need developers, designers, marketers, or product managers, CollabNest helps you find the perfect match.
      </p>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
      <!-- Talent Card 1 -->
      <div class="bg-white border rounded-lg shadow-md p-6 flex flex-col items-center text-center" data-aos="zoom-in" data-aos-delay="100">
        <img
          src="https://randomuser.me/api/portraits/women/44.jpg"
          alt="Talent 1"
          class="w-28 h-28 rounded-full mb-4 object-cover"
          loading="lazy"
        />
        <h3 class="text-xl font-semibold text-gray-900">Sophia Lee</h3>
        <p class="text-blue-600 font-medium mb-2">Full Stack Developer</p>
        <p class="text-gray-600 text-sm">
          Expert in React, Node.js, and cloud solutions. Passionate about building scalable web apps.
        </p>
        <div class="mt-4 flex space-x-3 text-blue-600">
          <i class="fab fa-js-square fa-lg"></i>
          <i class="fab fa-react fa-lg"></i>
          <i class="fab fa-node-js fa-lg"></i>
        </div>
      </div>

      <!-- Talent Card 2 -->
      <div class="bg-white border rounded-lg shadow-md p-6 flex flex-col items-center text-center" data-aos="zoom-in" data-aos-delay="200">
        <img
          src="https://randomuser.me/api/portraits/men/36.jpg"
          alt="Talent 2"
          class="w-28 h-28 rounded-full mb-4 object-cover"
          loading="lazy"
        />
        <h3 class="text-xl font-semibold text-gray-900">David Kim</h3>
        <p class="text-blue-600 font-medium mb-2">UI/UX Designer</p>
        <p class="text-gray-600 text-sm">
          Creates intuitive, user-friendly interfaces with a strong focus on user experience.
        </p>
        <div class="mt-4 flex space-x-3 text-blue-600">
          <i class="fab fa-sketch fa-lg"></i>
          <i class="fab fa-figma fa-lg"></i>
          <i class="fas fa-pencil-ruler fa-lg"></i>
        </div>
      </div>

      <!-- Talent Card 3 -->
      <div class="bg-white border rounded-lg shadow-md p-6 flex flex-col items-center text-center" data-aos="zoom-in" data-aos-delay="300">
        <img
          src="https://randomuser.me/api/portraits/women/68.jpg"
          alt="Talent 3"
          class="w-28 h-28 rounded-full mb-4 object-cover"
          loading="lazy"
        />
        <h3 class="text-xl font-semibold text-gray-900">Aisha Patel</h3>
        <p class="text-blue-600 font-medium mb-2">Digital Marketer</p>
        <p class="text-gray-600 text-sm">
          Specializes in SEO, content marketing, and social media campaigns to boost brand visibility.
        </p>
        <div class="mt-4 flex space-x-3 text-blue-600">
          <i class="fas fa-bullhorn fa-lg"></i>
          <i class="fab fa-google fa-lg"></i>
          <i class="fab fa-facebook fa-lg"></i>
        </div>
      </div>
    </div>
  </section>

  <!-- AOS JS -->
  <script src="https://unpkg.com/aos@2.3.1/dist/aos.js"></script>
  <script>
    AOS.init({
      duration: 800,
      easing: 'ease-in-out',
      once: true,
    });
  </script>
</body>
</html>
